...
export const handleBallOutOfBounds = (game: Game) => {
  ...
  const isOutLeft = Math.abs(ball.x) > mapBounds.x && Math.abs(ball.y) > goals.y && ball.x < 0;
  const isOutRight = Math.abs(ball.x) > mapBounds.x && Math.abs(ball.y) > goals.y && ball.x > 0;
  const isOutTopBottom = Math.abs(ball.y) > mapBounds.y && Math.abs(ball.x) < mapBounds.x;
...
export const freeKick = async (
  ...
  const defenders = room.getPlayerList().filter((p) => p.team != forTeam && p.team != 0);
  defenders.forEach(p => {
    room.setPlayerDiscProperties(p.id, {
...
const throwRealBall = async (
  ...
  const toMass = game.gameState === "penalty" ? defaults.ballInvMass :
    (game.rotateNextKick ? defaults.ballInvMass + 0.63 : defaults.ballInvMass);
  
  // Set ball color based on which team is taking the shot
  const ballColor = forTeam === 1 ? colors.red : colors.blue;
