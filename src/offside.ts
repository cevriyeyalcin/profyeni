...
import { offsideDiscs, mapBounds, defaults, getOffsideEnabled } from "./settings";
...
const checkOffside = async (game: Game, p: PlayerAugmented) => {
  // Check if offside system is enabled
  ...
  if (defenders.length >= 1) {  // En az 1 oyuncu önümde olmalı
  return; // ofsayt yok
}
  if (!game.inPlay) {
    return;
  ...
  if (room.getPlayerList().filter((p) => p.team != 0).length <= 6) {
    sendMessage("❌🚩 Altıdan az kişi bulunduğu için ofsayt yok.");
    return;
  }
